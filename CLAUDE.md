...
Ajouté en sept. 2026, uniquement pour ces deux fonctionnalités : le reste du site reste en `localStorage`. Vérifié de bout en bout en production avec l'effectif réel, sauf la connexion d'un parent réel (adresse autre que celle de Gregory) : jamais testée, et aujourd'hui impossible — voir « Email et connexion ».
...
**Email et connexion**
- Le service d'email par défaut de Supabase est utilisé. **Il ne délivre qu'aux adresses des membres de l'équipe du projet** et limite à 2 messages par heure (doc Supabase, « Send emails with custom SMTP ») : les tests n'ont marché que parce que l'adresse de Gregory en fait partie, et **aucun parent réel ne peut se connecter tant qu'un SMTP personnalisé n'est pas branché**. Ne jamais ajouter des parents à l'équipe Supabase : ils auraient accès au tableau de bord. `site_url` est `https://football-analysis-ten.vercel.app` et la liste blanche de redirections contient la prod et `http://localhost:5173/**` — sans ça, les liens magiques renvoyaient vers `localhost:3000`.
- Un SMTP personnalisé (Brevo) a été essayé puis retiré : l'expéditeur doit appartenir à un domaine authentifié — un relais privé Apple ou une adresse `sfr.fr` sont rejetés (DMARC). **Il manque un domaine que Gregory ou le club contrôle vraiment.** `gamechanger.com`, listé dans le compte Brevo (non authentifié), n'est pas à lui : il est enregistré depuis 2002 chez un registrar d'entreprise et porte une messagerie d'entreprise, et « Route 53 » n'était que le fournisseur DNS que Brevo y détecte — l'hypothèse inverse, notée ici auparavant, ne reposait sur aucune capture d'un compte AWS de Gregory. Ne jamais l'utiliser comme expéditeur. Pistes : un domaine du club (l'option « Déléguer » de Brevo envoie un lien sécurisé à la personne qui gère ses DNS) ou un domaine à une dizaine d'euros par an. Ensuite, générer une clé SMTP et la saisir directement dans Supabase (Authentication → SMTP), pas dans une conversation.
...
- **Bloquant avant d'inviter la moindre famille** : obtenir un domaine dont Gregory ou le club contrôle le DNS, l'authentifier chez Brevo (SPF/DKIM/DMARC), brancher le SMTP dans Supabase, puis tester la connexion avec une adresse qui n'est pas celle de Gregory (voir « Email et connexion »).
